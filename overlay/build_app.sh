#!/bin/bash
# Builds WisprFlow.app -- agent-style (no Dock icon, no menu bar item).
...
APP="$PWD/WisprFlow.app"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp .build/release/FlowOverlay "$APP/Contents/MacOS/WisprFlow"
cat > "$APP/Contents/Info.plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>CFBundleName</key><string>WisprFlow</string>
  <key>CFBundleDisplayName</key><string>WisprFlow</string>
  <key>CFBundleIdentifier</key><string>com.wisprflowclone.overlay</string>
  <key>CFBundleVersion</key><string>1</string>
  <key>CFBundleShortVersionString</key><string>1.0</string>
  <key>CFBundlePackageType</key><string>APPL</string>
  <key>CFBundleExecutable</key><string>WisprFlow</string>
  ...
  <string>WisprFlow records audio while you hold the dictation hotkey, and transcribes it on this Mac.</string>
</dict>
</plist>
PLIST
...
#     Name: WisprFlow Signing   Identity Type: Self Signed Root
#     Certificate Type: Code Signing
#   Then: export FLOW_SIGN_IDENTITY="WisprFlow Signing"
...
if [ "$IDENTITY" = "-" ]; then
  ...
  echo "      must RE-GRANT Accessibility + Input Monitoring to WisprFlow now:"
  echo "        1. tccutil reset Accessibility com.wisprflowclone.overlay"
  echo "        2. tccutil reset ListenEvent   com.wisprflowclone.overlay"
  echo "        3. re-add $APP in System Settings > Privacy & Security"
  echo "        4. ./flowctl restart"
  echo "      Avoid this permanently by creating a self-signed cert and setting"
  echo "      FLOW_SIGN_IDENTITY (see comments in overlay/build_app.sh)."
fi
# Drop the pre-rename bundle so there is only one app to grant permissions to.
rm -rf "$PWD/FlowOverlay.app"
echo "Built: $APP"
